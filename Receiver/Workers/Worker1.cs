...
public class Worker1 : IHostedService, IDisposable
{
    ...
    public Worker1(EventReceiver eventReceiver, ILogger<Worker1> logger)
    {
    ...
    {
        _logger.LogTrace("New event has been received. {@Event}", @event);
    ...
    public Task StartAsync(CancellationToken cancellationToken) =>
        ...
        {
            const string queueName = $"Pipeline.{RoutingKeys.RoutingKeyWorker1}";
            _eventReceiver.SubscribeQueue(queueName, RoutingKeys.RoutingKeyWorker1, MessageReceivedAsync);
        }, cancellationToken);
